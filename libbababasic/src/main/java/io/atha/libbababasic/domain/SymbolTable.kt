...
class SymbolTable {
    ...
    private fun getEntry(id: Int): STEntry {
        ...
        throw InternalError("Failed to find entry for id: $id")
    }
    ...
    fun getCompositeVariableIdForVariable(variableName: VariableName): Int {
        ...
        if (id == -1) {
            throw InternalError("Failed to find variable: $variableName")
        }
        return id
    }
    ...
    fun getDataTypeFor(varname: String, suffix: String?): PuffinBasicAtomTypeId {
        ...
        if (varname.isEmpty()) {
            throw InternalError("Empty variable name: $varname")
        }
    ...
    fun getStructType(name: String): StructType {
        return userDefinedTypes[name]
            ?: throw RuntimeError(
                RuntimeError.ErrorCode.MISSING_STRUCT,
                "Missing struct: $name"
            )
    }
    ...
    fun popScope() {
        val parent = currentScope!!.parent ?: throw InternalError("Scope underflow!")
        currentScope = parent
    }
...
fun variableConsumer(consumer: (id: Int, entry: STVariable, variable: Variable) -> Unit): SymbolTable.VariableConsumer {
    return object : SymbolTable.VariableConsumer {
